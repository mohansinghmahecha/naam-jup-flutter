...
class _AiDemoState extends ConsumerState<AiDemo>
  ...
  void _handleFinalResult(String finalText) {
    ...
    for (int i = 0; i <= textTokens.length - targetTokens.length; i++) {
      if (fuzzyTokensMatch(textTokens, i, targetTokens)) {
        matches++;
        debugPrint(
          '[DEBUG] fuzzy match at index $i -> window=${textTokens.sublist(i, i + targetTokens.length)}',
        );
    ...
    for (int i = 0; i < matches; i++) {
      ...
      debugPrint(
        '[DEBUG] incremented for ${_selectedGod!.name} (${i + 1}/$matches)',
      );
    }
  }
  ...
  void _toggleCountingPause() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          _isCountingPaused ? 'Counting paused' : 'Counting resumed',
        ),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    final God? selected = (gods.isEmpty)
        ? null
        : gods.firstWhere(
            (g) => g.id == _selectedGod?.id,
            orElse: () => gods.first,
          );

    // Only update selected if null or changed externally
    if (_selectedGod == null ||
        (selected != null && selected.id != _selectedGod!.id)) {
              ...
              child: Row(
                children: [
                  ...
                  ElevatedButton.icon(
                    ...
                    icon: Icon(
                      _isCountingPaused ? Icons.play_arrow : Icons.pause,
                      color: Colors.white,
                    ),
                    label: Text(
                      _isCountingPaused ? 'RESUME' : 'STOP',
                      style: const TextStyle(color: Colors.white),
                    ),
                    style: ElevatedButton.styleFrom(
                      backgroundColor: _isCountingPaused
                          ? Colors.grey[800]
                          : Colors.black,
                    ),
                  ),
                      ...
                      child: Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          gradient: LinearGradient(
                            ...
                            colors: _isListening
                                ? [Colors.white, Colors.orange.shade300]
                                : [Colors.white70, Colors.orange.shade800],
                          ),
                          boxShadow: [
                            BoxShadow(
